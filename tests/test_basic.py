...
class TestModuleImports:
    ...
    @pytest.mark.skipif(
        not modbuslink_available,
        reason=f"ModbusLink模块不可用 | ModbusLink modules not available: {import_error if not modbuslink_available else ''}",
    )
...
class TestModbusClient:
    ...
    def test_client_connect_tcp(self, mock_socket):
        ...
        try:
            ...
            print(
                f"TCP连接测试异常（预期）: {e} | TCP connection test exception (expected): {e}"
            )
        finally:
            try:
                client.disconnect()
...
class TestCRC16Modbus:
    ...
    def test_crc_calculation(self):
        ...
        print(
            f"✓ CRC计算正确: {calculated_crc:04X} | CRC calculation correct: {calculated_crc:04X}"
        )
...
class TestPayloadCoder:
    ...
    def test_encode_decode_float32(self):
        ...
        print(
            f"✓ Float32编码解码正常: {original_value} -> {decoded_value} | Float32 encode/decode works: {original_value} -> {decoded_value}"
        )
    ...
    def test_encode_decode_int32(self):
        ...
        print(
            f"✓ Int32编码解码正常: {original_value} -> {decoded_value} | Int32 encode/decode works: {original_value} -> {decoded_value}"
        )
    ...
    def test_encode_decode_string(self):
        ...
        print(
            f"✓ 字符串编码解码正常: '{original_string}' | String encode/decode works: '{original_string}'"
        )
    ...
    def test_different_byte_orders(self):
        ...
        print(
            f"✓ 不同字序处理正常: 高字在前={registers_big_high}, 低字在前={registers_big_low} | Different word order handling works: high first={registers_big_high}, low first={registers_big_low}"
        )
...
class TestTransportErrorHandling:
    ...
    def test_transport_connection_errors(self):
        ...
        tcp_transport = TcpTransport(
            host="192.0.2.1", port=12345, timeout=0.1
        )  # 使用不存在的地址 | Use non-existent address

        with pytest.raises(
            Exception
        ):  # 应该抛出连接异常 | Should raise connection exception
            tcp_transport.connect()

        print(
            "✓ 传输层连接错误处理正常 | Transport layer connection error handling works"
        )
    ...
    def test_transport_send_without_connection(self):
        ...
        with pytest.raises(
            Exception
        ):  # 应该抛出未连接异常 | Should raise not connected exception
