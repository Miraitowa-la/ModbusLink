...
class TestCRC16ModbusCalculation:
    ...
    def test_empty_data(self):
        ...
        assert (
            crc == 0xFFFF
        )  # 空数据的CRC应该是初始值 | CRC for empty data should be initial value
        print(
            f"✓ 空数据CRC计算正确: {crc:04X} | Empty data CRC calculation correct: {crc:04X}"
        )
    ...
    def test_single_byte(self):
        ...
        for data, expected in test_cases:
            ...
            assert crc == expected
            print(
                f"✓ 单字节 {data.hex().upper()} CRC: {crc:04X} | Single byte {data.hex().upper()} CRC: {crc:04X}"
            )
    ...
    def test_known_test_vectors(self):
        ...
        test_vectors = [
            ...
            (
                ...
                0xCDC5,
            ),  # 读保持寄存器请求 | Read holding registers request
            (
                b"\x01\x04\x00\x00\x00\x0a",
                0x0D70,
            ),  # 读输入寄存器请求 | Read input registers request
            (
                b"\x01\x06\x00\x00\x00\x01",
                0x0A48,
            ),  # 写单个寄存器请求 | Write single register request
            (
                b"\x01\x10\x00\x00\x00\x02\x04\x00\x0a\x00\x0b",
                0x6A92,
            ),  # 写多个寄存器请求 | Write multiple registers request
        ]
        ...
        for data, expected_crc in test_vectors:
            ...
            print(
                f"✓ 测试向量 {data.hex().upper()} CRC: {calculated_crc:04X} | Test vector {data.hex().upper()} CRC: {calculated_crc:04X}"
            )
    ...
    def test_incremental_calculation(self):
        ...
        print(
            f"✓ CRC计算一致性验证: {crc_all:04X} == {crc_verify:04X} | CRC calculation consistency verified: {crc_all:04X} == {crc_verify:04X}"
        )
    ...
    def test_large_data(self):
        ...
        assert 0 <= crc <= 0xFFFF
        print(
            f"✓ 大数据块CRC计算完成: {crc:04X} (数据长度: {len(large_data)}) | Large data block CRC calculated: {crc:04X} (data length: {len(large_data)})"
        )
...
class TestCRC16ModbusVerification:
    ...
    def test_verify_valid_packets(self):
        ...
        for packet in valid_packets:
            ...
            print(
                f"✓ 有效数据包验证通过: {packet.hex().upper()} | Valid packet verification passed: {packet.hex().upper()}"
            )
    ...
    def test_verify_invalid_packets(self):
        ...
        for packet in invalid_packets:
            ...
            print(
                f"✓ 无效数据包正确识别: {packet.hex().upper()} | Invalid packet correctly identified: {packet.hex().upper()}"
            )
    ...
    def test_verify_short_packets(self):
        ...
        for packet in short_packets:
            ...
            assert is_valid is False
            print(
                f"✓ 过短数据包正确拒绝: {packet.hex().upper() if packet else '(空)'} | Short packet correctly rejected: {packet.hex().upper() if packet else '(empty)'}"
            )
...
class TestCRC16ModbusEdgeCases:
    ...
    def test_crc_consistency(self):
        ...
        print(
            f"✓ CRC计算一致性验证通过: {crc_int:04X} | CRC calculation consistency verified: {crc_int:04X}"
        )
...
class TestCRC16ModbusPerformance:
    ...
    def test_performance_small_data(self):
        ...
        print(
            f"✓ 小数据块性能: {rate:.0f} 次/秒 ({elapsed:.3f}秒/{iterations}次) | Small data performance: {rate:.0f} ops/sec ({elapsed:.3f}s/{iterations} ops)"
        )
    ...
    def test_performance_large_data(self):
        ...
        print(
            f"✓ 大数据块性能: {rate:.0f} 次/秒, {throughput:.1f} MB/s | Large data performance: {rate:.0f} ops/sec, {throughput:.1f} MB/s"
        )
...
if __name__ == "__main__":
    print(
        "开始运行CRC16 Modbus功能测试... | Starting CRC16 Modbus functionality tests..."
    )
    print("=" * 70)
