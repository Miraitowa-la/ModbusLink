...
try:
    ...
    pytest.skip(
        f"无法导入异步模块: {e} | Cannot import async modules: {e}",
...
class TestAsyncTcpTransport:
    ...
    def test_async_transport_invalid_params(self):
        ...
        print(
            "✓ 异步TCP传输层参数验证正常 | Async TCP transport parameter validation works"
        )
    ...
    async def test_async_transport_context_manager(self):
        ...
        try:
            ...
            print(
                f"✓ 异步传输层上下文管理器协议正常（连接失败是预期的）: {e} | Async transport context manager protocol works (connection failure expected): {e}"
            )
...
class TestAsyncModbusClient:
    ...
    async def test_async_client_context_manager(self):
        ...
        try:
            ...
            print(
                f"✓ 异步客户端上下文管理器协议正常（连接失败是预期的）: {e} | Async client context manager protocol works (connection failure expected): {e}"
            )
    ...
    def test_async_client_advanced_methods_exist(self):
        ...
        for method_name in advanced_methods:
            if hasattr(client, method_name):
                ...
                assert asyncio.iscoroutinefunction(method)
                print(
                    f"✓ 异步高级方法存在: {method_name} | Async advanced method exists: {method_name}"
                )
        ...
        try:
            async with client:
                ...
                result = await client.read_holding_registers(
                    slave_id=1, start_address=0, quantity=2
                )
                print(
                    f"✓ 异步模拟操作成功: {result} | Async mock operation successful: {result}"
                )
        except Exception as e:
            print(
                f"✓ 异步操作测试完成（异常是预期的）: {e} | Async operation test completed (exception expected): {e}"
            )
...
class TestDataStore:
    ...
    def test_holding_registers_operations(self):
        ...
        print(
            f"✓ 保持寄存器操作正常: {test_values} -> {retrieved_values} | Holding registers operation works: {test_values} -> {retrieved_values}"
        )
    ...
    def test_coils_operations(self):
        ...
        print(
            f"✓ 线圈操作正常: {test_coils} -> {retrieved_coils} | Coils operation works: {test_coils} -> {retrieved_coils}"
        )
    ...
    def test_input_registers_operations(self):
        ...
        assert retrieved_values == test_values
        print(
            f"✓ 输入寄存器操作正常: {test_values} -> {retrieved_values} | Input registers operation works: {test_values} -> {retrieved_values}"
        )
    ...
    def test_discrete_inputs_operations(self):
        ...
        print(
            f"✓ 离散输入操作正常: {test_inputs} -> {retrieved_inputs} | Discrete inputs operation works: {test_inputs} -> {retrieved_inputs}"
        )
...
class TestModbusSlave:
    ...
    def test_slave_context_manager(self):
        ...
        try:
            ...
            print(
                f"✓ 从站上下文管理器测试完成: {e} | Slave context manager test completed: {e}"
            )
...
class TestAsyncIntegration:
    ...
    async def test_async_timeout_handling(self):
        ...
        try:
            ...
            print(
                f"✓ 异步超时处理正常: {type(e).__name__}: {e} | Async timeout handling works: {type(e).__name__}: {e}"
            )
        except Exception as e:
            print(
                f"✓ 异步异常处理正常: {type(e).__name__}: {e} | Async exception handling works: {type(e).__name__}: {e}"
            )
    ...
    async def test_concurrent_operations(self):
        ...
        # transport = AsyncTcpTransport(host="127.0.0.1", port=502, timeout=1.0)
        # client = AsyncModbusClient(transport)  # 暂时不使用 | Not used for now
        ...
        assert (
            elapsed_time < 0.5
        )  # 并发执行应该比顺序执行快 | Concurrent execution should be faster than sequential

        print(
            f"✓ 并发操作测试通过: {len(results)}个任务在{elapsed_time:.3f}秒内完成 | Concurrent operations test passed: {len(results)} tasks completed in {elapsed_time:.3f}s"
        )
    ...
    async def test_callback_mechanism(self):
        ...
        assert (
            "回调收到" in callback_results[0]
            or "Callback received" in callback_results[0]
        )
        assert "处理了数据" in result or "Processed data" in result

        print(
            f"✓ 回调机制测试通过: {callback_results[0]} | Callback mechanism test passed: {callback_results[0]}"
        )
...
class TestAsyncPerformance:
    ...
    async def test_async_vs_sync_simulation(self):
        ...
        async_tasks = [
            async_operation(operation_duration) for _ in range(num_operations)
        ]
        ...
        assert async_elapsed < (
            operation_duration * num_operations
        )  # 应该比顺序执行快 | Should be faster than sequential

        print(
            f"✓ 异步性能测试: {num_operations}个操作在{async_elapsed:.3f}秒内完成 | Async performance test: {num_operations} operations completed in {async_elapsed:.3f}s"
        )
        print(
            f"  理论顺序执行时间: {operation_duration * num_operations:.3f}秒 | Theoretical sequential time: {operation_duration * num_operations:.3f}s"
        )
        print(
            f"  性能提升: {((operation_duration * num_operations) / async_elapsed):.1f}倍 | Performance improvement: {((operation_duration * num_operations) / async_elapsed):.1f}x"
        )
