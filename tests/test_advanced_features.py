...
try:
    ...
    pytest.skip(
        f"无法导入高级功能模块: {e} | Cannot import advanced feature modules: {e}",
...
class TestPayloadCoderFloat32:
        ...
        for value in test_values:
            for byte_order in ["big", "little"]:
                for word_order in ["high", "low"]:
                    registers = PayloadCoder.encode_float32(
                        value, byte_order, word_order
                    )
                    decoded = PayloadCoder.decode_float32(
                    ...
                    print(
                        f"✓ Float32 {value} ({byte_order}/{word_order}): {decoded} | Float32 {value} ({byte_order}/{word_order}): {decoded}"
                    )
    ...
    def test_encode_decode_negative_float(self):
        ...
        for value in test_values:
            ...
            assert abs(decoded - value) < 0.1
            print(
                f"✓ 负Float32 {value}: {decoded} | Negative Float32 {value}: {decoded}"
            )
    ...
    def test_encode_decode_special_float_values(self):
        ...
        for value in special_values:
            ...
            print(
                f"✓ 特殊Float32 {value}: {decoded} | Special Float32 {value}: {decoded}"
            )
...
class TestPayloadCoderIntegers:
        ...
        for value in test_values:
            for byte_order in ["big", "little"]:
                for word_order in ["high", "low"]:
                    ...
                    decoded = PayloadCoder.decode_int32(
                        registers, byte_order, word_order
                    )
                    ...
                    assert decoded == value
                    print(
                        f"✓ Int32 {value} ({byte_order}/{word_order}): {decoded} | Int32 {value} ({byte_order}/{word_order}): {decoded}"
                    )
    ...
    def test_encode_decode_uint32(self):
        ...
        for value in test_values:
            for byte_order in ["big", "little"]:
                for word_order in ["high", "low"]:
                    registers = PayloadCoder.encode_uint32(
                        value, byte_order, word_order
                    )
                    decoded = PayloadCoder.decode_uint32(
                        registers, byte_order, word_order
                    )
                    ...
                    assert decoded == value
                    print(
                        f"✓ UInt32 {value} ({byte_order}/{word_order}): {decoded} | UInt32 {value} ({byte_order}/{word_order}): {decoded}"
                    )
    ...
    def test_integer_overflow_handling(self):
        ...
        with pytest.raises((ValueError, OverflowError, struct.error)):
            PayloadCoder.encode_int32(
                2147483648, "big", "high"
            )  # 超出int32范围 | Beyond int32 range

        with pytest.raises((ValueError, OverflowError, struct.error)):
            PayloadCoder.encode_uint32(
                -1, "big", "high"
            )  # 负数不能编码为uint32 | Negative cannot be encoded as uint32
    ...
    def test_invalid_register_count_errors(self):
        ...
        with pytest.raises(ValueError, match="需要恰好2个寄存器"):
            PayloadCoder.decode_float32(
                [1], "big", "high"
            )  # 只有1个寄存器 | Only 1 register
        ...
        with pytest.raises(ValueError, match="需要恰好2个寄存器"):
            PayloadCoder.decode_int32(
                [1], "big", "high"
            )  # 只有1个寄存器 | Only 1 register

        # 测试int64解码错误 | Test int64 decoding error
        with pytest.raises(ValueError, match="需要恰好4个寄存器"):
            PayloadCoder.decode_int64(
                [1, 2], "big", "high"
            )  # 只有2个寄存器 | Only 2 registers

        print(
            "✓ 无效寄存器数量错误处理正常 | Invalid register count error handling works"
        )
    ...
    def test_string_encoding_edge_cases(self):
        ...
        try:
            ...
            print(
                "✓ 长字符串处理正常（可能被截断） | Long string handled (possibly truncated)"
            )
        except ValueError:
        ...
        try:
            ...
            print(
                f"✓ 无效编码错误处理正常: {type(e).__name__} | Invalid encoding error handled: {type(e).__name__}"
            )

        print(
            "✓ 字符串编码边界情况处理正常 | String encoding edge cases handled correctly"
        )
...
class TestPayloadCoderString:
    ...
    def test_encode_decode_ascii_string(self):
        ...
        for string in test_strings:
            ...
            print(
                f"✓ ASCII字符串 '{string}': 长度{len(registers)}寄存器 | ASCII string '{string}': {len(registers)} registers"
            )
    ...
    def test_encode_decode_unicode_string(self):
        ...
        for string in test_strings:
            ...
            assert decoded.rstrip("\x00") == string
            print(
                f"✓ Unicode字符串 '{string}': {len(registers)}寄存器, {byte_length}字节 | Unicode string '{string}': {len(registers)} registers, {byte_length} bytes"
            )
    ...
    def test_string_padding(self):
        ...
        string = "Test"
        register_count = (
            5  # 足够容纳字符串和填充 | Enough to contain string and padding
        )
        registers = PayloadCoder.encode_string(string, register_count)
        ...
        print(
            f"✓ 字符串填充正常: '{string}' -> {len(registers)}寄存器 | String padding works: '{string}' -> {len(registers)} registers"
        )
    ...
    def test_string_truncation(self):
        ...
        null_char = "\x00"
        assert decoded_full.rstrip(null_char) == string
        print(
            f"✓ 字符串编码解码正常: '{string}' -> '{decoded_full.rstrip(null_char)}' | String encoding/decoding works: '{string}' -> '{decoded_full.rstrip(null_char)}'"
        )
...
class TestPayloadCoderByteOrder:
    ...
    def test_different_byte_orders_produce_different_results(self):
        ...
        print(
            f"✓ 字节序测试: 大端{registers_big}, 小端{registers_little} | Byte order test: big-endian{registers_big}, little-endian{registers_little}"
        )
    ...
    def test_different_word_orders_produce_different_results(self):
        ...
        assert registers_high == [
            registers_low[1],
            registers_low[0],
        ]  # 应该是相反的顺序 | Should be reverse order
        ...
        print(
            f"✓ 字序测试: 高字在前{registers_high}, 低字在前{registers_low} | Word order test: high-first{registers_high}, low-first{registers_low}"
        )
    ...
    def test_cross_decode_fails(self):
        ...
        print(
            f"✓ 交叉解码检测: 原值{value:08X}, 错误解码{decoded_wrong:08X} | Cross-decode detection: original{value:08X}, wrong decode{decoded_wrong:08X}"
        )
...
class TestModbusLogger:
    ...
    def test_protocol_debug(self):
        ...
        logger.debug(
            f"发送数据: {test_data.hex().upper()} | Sending data: {test_data.hex().upper()}"
        )
...
class TestAdvancedClientFeatures:
    ...
    def test_client_advanced_data_types(self, mock_socket):
        ...
        for method_name in advanced_methods:
            ...
            print(
                f"✓ 高级方法存在: {method_name} | Advanced method exists: {method_name}"
            )
    ...
    def test_client_method_signatures(self):
        ...
        for method_name, expected_params in method_signatures.items():
            if hasattr(client, method_name):
                ...
                print(
                    f"✓ 方法签名正确: {method_name}{list(sig.parameters.keys())} | Method signature correct: {method_name}{list(sig.parameters.keys())}"
                )
