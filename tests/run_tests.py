...
def check_dependencies():
    ...
    if missing_packages:
        ...
        print("请运行 | Please run: pip install pytest pytest-cov pytest-mock pytest-asyncio")
        return False
...
示例 | Examples:
  ...
  python run_tests.py --performance      # 运行性能测试 | Run performance tests
    ...
    test_group.add_argument('--basic', action='store_true', help='只运行基础测试 | Run only basic tests')
    test_group.add_argument('--crc', action='store_true', help='只运行CRC测试 | Run only CRC tests')
    test_group.add_argument('--advanced', action='store_true', help='只运行高级功能测试 | Run only advanced features tests')
    test_group.add_argument('--async-tests', action='store_true', help='只运行异步测试 | Run only async tests')
    test_group.add_argument('--integration', action='store_true', help='只运行集成测试 | Run only integration tests')
    test_group.add_argument('--coverage', action='store_true', help='运行覆盖率测试 | Run coverage tests')
    test_group.add_argument('--performance', action='store_true', help='运行性能测试 | Run performance tests')
    
    # 输出选项 | Output options
    output_group = parser.add_mutually_exclusive_group()
    output_group.add_argument('--verbose', '-v', action='store_true', help='详细输出 | Verbose output')
    output_group.add_argument('--quiet', '-q', action='store_true', help='安静模式 | Quiet mode')
