"""ModbusLink 传输层抽象基类 ModbusLink Transport Layer Abstract Base Class

定义了所有传输层实现必须遵循的统一接口。 Defines the unified interface that all transport layer implementations must follow.
"""
...
class BaseTransport(ABC):
    """传输层抽象基类 Transport Layer Abstract Base Class
    ...
    @abstractmethod
    def open(self) -> None:
        """打开传输连接 Open Transport Connection
    ...
    @abstractmethod
    def close(self) -> None:
        """关闭传输连接 Close Transport Connection
