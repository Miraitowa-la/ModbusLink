"""ModbusLink RTU传输层实现 RTU Transport Layer Implementation
...
class RtuTransport(BaseTransport):
    """Modbus RTU传输层实现 Modbus RTU Transport Layer Implementation
    
    处理基于串口的Modbus RTU通信，包括： Handles Modbus RTU communication based on serial port, including:
    ...
    def send_and_receive(self, slave_id: int, pdu: bytes) -> bytes:
        """发送PDU并接收响应 Send PDU and receive response
        
        实现RTU协议的完整通信流程： Implements complete RTU protocol communication flow:
    ...
    def _receive_response(self, expected_slave_id: int, function_code: int) -> bytes:
        """接收完整的响应帧 Receive complete response frame
        
        根据功能码预估响应长度，智能接收数据。 Estimate response length based on function code and intelligently receive data.
        """
