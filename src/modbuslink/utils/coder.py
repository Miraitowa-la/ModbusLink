...
class PayloadCoder:
    ...
    def encode_int64(value: int, byte_order: str = BIG_ENDIAN, word_order: str = HIGH_WORD_FIRST, signed: bool = True) -> List[int]:
        ...
        Returns:
            List[int]: 包含四个16位寄存器值的列表 List containing four 16-bit register values
        """
        # 将整数编码为字节 Encode integer to bytes
        format_char = '>q' if byte_order == PayloadCoder.BIG_ENDIAN else '<q'
        if not signed:
    ...
    def decode_string(registers: List[int], byte_order: str = BIG_ENDIAN, encoding: str = 'utf-8') -> str:
        ...
        Returns:
            str: 解码后的字符串（去除尾部空字符） Decoded string (with trailing null characters removed)
        """
        # 将所有寄存器值转换为字节 Convert all register values to bytes
        all_bytes = b''
    ...
    def encode_string(value: str, register_count: int, byte_order: str = BIG_ENDIAN, encoding: str = 'utf-8') -> List[int]:
        ...
        Returns:
            List[int]: 包含字符串数据的寄存器列表 List of registers containing string data
