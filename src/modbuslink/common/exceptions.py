"""ModbusLink 异常定义模块 Exception Definition Module

定义了所有ModbusLink库中使用的异常类型。 Defines all exception types used in the ModbusLink library.
"""


class ModbusLinkError(Exception):
    """ModbusLink库的基础异常类 Base exception class for ModbusLink library
    
    所有ModbusLink相关的异常都继承自这个基类。 All ModbusLink-related exceptions inherit from this base class.
    """
    pass


class ConnectionError(ModbusLinkError):
    """连接错误异常 Connection error exception
    
    当无法建立或维持与Modbus设备的连接时抛出。 Raised when unable to establish or maintain connection with Modbus device.
    """
    pass


class TimeoutError(ModbusLinkError):
    """超时错误异常 Timeout error exception
    
    当操作超过指定的超时时间时抛出。 Raised when operation exceeds the specified timeout period.
    """
    pass


class CRCError(ModbusLinkError):
    """CRC校验错误异常 CRC validation error exception
    
    当接收到的数据帧CRC校验失败时抛出。 Raised when CRC validation of received data frame fails.
    """
    pass


class InvalidResponseError(ModbusLinkError):
    """无效响应错误异常 Invalid response error exception
    
    当接收到的响应格式不正确或不符合预期时抛出。 Raised when received response format is incorrect or unexpected.
    """
    pass


class ModbusException(ModbusLinkError):
    """Modbus协议异常 Modbus protocol exception
    
    当从站返回Modbus异常码时抛出。 Raised when slave returns Modbus exception code.
