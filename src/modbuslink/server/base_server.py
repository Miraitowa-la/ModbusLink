...
from ..common.exceptions import ModbusException, InvalidReplyError
...
class AsyncBaseModbusServer(ABC):
    ...
    def __init__(
        ...
        self._function_handlers: Dict[int, Callable[[bytes], bytes]] = {
    ...
    def process_request(self, slave_id: int, pdu: bytes) -> bytes:
        ...
        try:
            ...
            self._logger.debug(
                ...
                en=f"Processing request: Function Code 0x{function_code:02X}, Data Length {len(data)}"
            )

            # 查找功能码处理器 | Find function code handler
            if function_code in self._function_handlers:
                response_pdu = self._function_handlers[function_code](data)
                self._logger.debug(
            ...
            return self._create_exception_response(pdu[0] if len(pdu) > 0 else 0x00, 0x04)

    @staticmethod
    def _create_exception_response(function_code: int, exception_code: int) -> bytes:
        """
        创建异常响应PDU
    ...
    def _handle_read_holding_registers(self, data: bytes) -> bytes:
        """
        处理读取保持寄存器请求（功能码0x03）
    ...
    def _handle_read_input_registers(self, data: bytes) -> bytes:
        """
        处理读取输入寄存器请求（功能码0x04）
    ...
    def _handle_write_multiple_coils(self, data: bytes) -> bytes:
        """
        处理写入多个线圈请求（功能码0x0F）
