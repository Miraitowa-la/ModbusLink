...
class DataStore:
    ...
    def get_discrete_inputs(self, start_address: int, quantity: int) -> List[bool]:
        ...
        return [
            self.discrete_inputs.get(start_address + i, False) for i in range(quantity)
        ]
    ...
    def set_holding_registers(self, start_address: int, values: List[int]) -> None:
        ...
        for i, value in enumerate(values):
            if not (0 <= value <= 65535):
                raise ValueError(
                    f"寄存器值必须在0-65535之间: {value} | Register value must be between 0-65535: {value}"
                )
    ...
    def get_holding_registers(self, start_address: int, quantity: int) -> List[int]:
        ...
        return [
            self.holding_registers.get(start_address + i, 0) for i in range(quantity)
        ]
...
class ModbusSlave:
    ...
    def start_tcp_server(self, host: str = "127.0.0.1", port: int = 502) -> None:
        ...
        self._logger.info(
            f"TCP从站服务器已启动，监听 {host}:{port} | TCP slave server started, listening on {host}:{port}"
        )
    ...
    def start_rtu_server(
        ...
        self._running = True
        self._server_thread = threading.Thread(
            target=self._rtu_server_loop, daemon=True
        )
        self._server_thread.start()

        self._logger.info(
            f"RTU从站服务器已启动，串口 {port} | RTU slave server started, serial port {port}"
        )
        ...
        try:
            ...
            while self._running:
                try:
                    ...
                    self._logger.debug(
                        f"客户端连接: {client_address} | Client connected: {client_address}"
                    )
                    ...
                    if self._running:
                        self._logger.error(
                            f"TCP服务器错误: {e} | TCP server error: {e}"
                        )
                    break

        except Exception as e:
            self._logger.error(
                f"TCP服务器启动失败: {e} | TCP server startup failed: {e}"
            )
                ...
                try:
                    ...
                    if protocol_id != 0:
                        self._logger.warning(
                            f"无效的协议ID: {protocol_id} | Invalid protocol ID: {protocol_id}"
                        )
                        continue

                    if unit_id != self.slave_id:
                        self._logger.debug(
                            f"忽略非本站请求: {unit_id} | Ignoring request for other slave: {unit_id}"
                        )
                        continue
                    ...
                    pdu_length = (
                        length - 1
                    )  # 减去unit_id的1字节 | Subtract 1 byte for unit_id
                    pdu = self._receive_exact(client_socket, pdu_length)
                    if not pdu:
                        break

                    self._logger.debug(
                        f"收到TCP请求: {pdu.hex()} | Received TCP request: {pdu.hex()}"
                    )
                    ...
                    response_length = (
                        len(response_pdu) + 1
                    )  # 加上unit_id的1字节 | Add 1 byte for unit_id
                    ...
                    self._logger.debug(
                        f"发送TCP响应: {response_pdu.hex()} | Sent TCP response: {response_pdu.hex()}"
                    )
                    ...
                    self._logger.error(
                        f"处理TCP客户端错误: {e} | Handle TCP client error: {e}"
                    )
                    break
        ...
        try:
            while self._running:
                try:
                    ...
                    if (
                        len(data) < 4
                    ):  # 最小帧长度：地址+功能码+CRC | Minimum frame length: address+function+CRC
                        continue

                    # 验证CRC | Verify CRC
                    if not CRC16Modbus.validate(data):
                        self._logger.warning(
                            f"CRC校验失败: {data.hex()} | CRC validation failed: {data.hex()}"
                        )
                        continue
                    ...
                    if slave_address != self.slave_id:
                        self._logger.debug(
                            f"忽略非本站请求: {slave_address} | Ignoring request for other slave: {slave_address}"
                        )
                        continue

                    self._logger.debug(
                        f"收到RTU请求: {pdu.hex()} | Received RTU request: {pdu.hex()}"
                    )
                    ...
                    if self._running:
                        self._logger.error(
                            f"RTU服务器错误: {e} | RTU server error: {e}"
                        )
                    time.sleep(0.1)
