...
class AsyncTcpModbusServer(AsyncBaseModbusServer):
    ...
    def __init__(
        ...
        self._logger.info(
            cn=f"TCP服务器初始化: {host}:{port}",
            en=f"TCP server initialized: {host}:{port}")
        ...
        try:
            while self._running:
                try:
                    ...
                    mbap_header = await asyncio.wait_for(reader.readexactly(7), timeout=30.0)
                    ...
                    pdu_data = await asyncio.wait_for(reader.readexactly(pdu_length), timeout=10.0)
                    ...
                    self._logger.debug(
                        cn=f"客户端断开连接(EOF): {client_addr}",
                        en=f"Client EOF: {client_addr}")
                    break
