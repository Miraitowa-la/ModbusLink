...
def main():
    ...
    print(
        f"TCP配置 | TCP configuration: {host}:{port}, 超时 | timeout: {timeout}秒 | seconds"
    )
    ...
    try:
        ...
        with client:
            ...
            try:
                registers = client.read_holding_registers(
                    slave_id=slave_id, start_address=0, quantity=10
                )
            ...
            try:
                registers = client.read_input_registers(
                    slave_id=slave_id, start_address=0, quantity=5
                )
            ...
            try:
                inputs = client.read_discrete_inputs(
                    slave_id=slave_id, start_address=0, quantity=16
                )
            ...
            try:
                ...
                client.write_multiple_coils(
                    slave_id=slave_id, start_address=0, values=coil_values
                )
            ...
            try:
                # 假设寄存器100存储温度值（放大100倍的整数） | Assume register 100 stores temperature value (integer scaled by 100)
                temp_registers = client.read_input_registers(
                    slave_id=slave_id, start_address=100, quantity=1
                )
                if temp_registers:
                    # 将整数值转换为浮点温度值 | Convert integer value to floating point temperature
                    temperature = temp_registers[0] / 100.0
                    print(
                        f"温度传感器读数 | Temperature sensor reading: {temperature:.2f}°C"
                    )
            ...
            try:
                ...
                client.write_float32(
                    slave_id=slave_id, start_address=200, value=temperature_setpoint
                )
                print(
                ...
                read_temperature = client.read_float32(
                    slave_id=slave_id, start_address=200
                )
                ...
                client.write_int32(
                    slave_id=slave_id, start_address=202, value=counter_value
                )
                print(f"✓ 写入32位整数 | Wrote 32-bit integer: {counter_value}")

                # 读取32位整数 | Read 32-bit integer
                read_counter = client.read_int32(slave_id=slave_id, start_address=202)
                print(f"读取32位整数 | Read 32-bit integer: {read_counter}")

                # 写入字符串 | Write string
                device_name = "ModbusLink"
                client.write_string(
                    slave_id=slave_id, start_address=210, value=device_name
                )
        ...
        print("请检查 | Please check:")
        print(
            "  - 设备是否正确实现Modbus TCP协议 | Device correctly implements Modbus TCP protocol"
        )
