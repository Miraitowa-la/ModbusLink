...
async def basic_operations_example():
    ...
    async with AsyncModbusClient(transport) as client:
        try:
            ...
            registers = await client.read_holding_registers(slave_id=1, start_address=0, quantity=4)
            ...
            value = await client.read_holding_registers(slave_id=1, start_address=0, quantity=1)
            ...
            await client.write_multiple_registers(slave_id=1, start_address=10, values=values)
            print(f"   写入值 | Written values: {values}")
            
            # 读取多个寄存器 | Read multiple registers
            print("\n5. 读取多个寄存器 | Reading multiple registers...")
            read_values = await client.read_holding_registers(slave_id=1, start_address=10, quantity=4)
            ...
            await client.write_multiple_coils(slave_id=1, start_address=0, values=coil_values)
            coils = await client.read_coils(slave_id=1, start_address=0, quantity=8)
...
async def advanced_data_types_example():
    ...
    async with AsyncModbusClient(transport) as client:
        try:
            ...
            await client.write_float32(slave_id=1, start_address=30, value=test_value, 
                                     byte_order='big', word_order='high')
            read_val1 = await client.read_float32(slave_id=1, start_address=30, 
                                                byte_order='big', word_order='high')
            print(f"   Big/High: 写入 {test_value}, 读取 {read_val1} | Written {test_value}, Read {read_val1}")
            
            # Little endian, low word first
            await client.write_float32(slave_id=1, start_address=32, value=test_value, 
                                     byte_order='little', word_order='low')
            read_val2 = await client.read_float32(slave_id=1, start_address=32, 
                                                byte_order='little', word_order='low')
            print(f"   Little/Low: 写入 {test_value}, 读取 {read_val2} | Written {test_value}, Read {read_val2}")
...
async def callback_example():
    ...
    def on_register_read(registers):
        print(f"   [回调] 读取到寄存器值 | [Callback] Read register values: {registers}")
...
async def concurrent_operations_example():
    ...
    async with AsyncModbusClient(transport) as client:
        try:
            ...
            print(f"   并发执行耗时 | Concurrent execution time: {end_time - start_time:.3f}秒")
